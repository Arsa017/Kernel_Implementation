...
PCB::PCB(Thread* t, StackSize _stackSize, Time _timeSlice) {
	...
	this->waitToCompleteList = new Lista();
	blockFlag = 1;
	id = nID++;
	myThread = t;
	stack = 0;
	PCBelem* element = new PCBelem(t, id); // pravimo objekat klase PCBelem, koji se sastoji iz niti i njenog ID-ja
...
void PCB::stackCreate() {
	...
	stack[stackSize-2] = FP_SEG(PCB::wrapper); // segmentni deo adrese f-je koju izvrsava nit
	stack[stackSize-3] = FP_OFF(PCB::wrapper); // offsetni deo adrese f-je
...
void PCB::wrapper() {
	...
	int n = (int) PCB::running->waitToCompleteList->brojElemenataListe();
